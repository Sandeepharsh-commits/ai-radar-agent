"""Fixed-page crawl with robots.txt, allowlist, and content hashing."""
...
def fetch_crawl_updates(state: dict) -> list[dict]:
    ...
    for page_name, url in CRAWL_PAGES.items():
        canon = normalize_url(url) or url
        if not is_allowed_url(canon):
            print(f"[skipped] domain not allowed: {page_name}")
            continue
        ...
        previous_hash = hashes.get(url) or hashes.get(canon)
